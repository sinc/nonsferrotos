...
def recalcMagnFieldinXY(field,sensorCoords):
    ...
    approx = RectBivariateSpline(y_grid, x_grid, np.array(field.Bx))
    Xnew,Ynew = np.meshgrid(x_grid,y_grid)
    data_X = [[approx(y,x) for x in x_grid] for y in y_grid]
    ...
    approx = RectBivariateSpline(y_grid, x_grid, np.array(field.By))
    Xnew,Ynew = np.meshgrid(x_grid,y_grid)
    data_Y = [[approx(y,x) for x in x_grid] for y in y_grid]
